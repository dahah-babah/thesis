...
@inject('workStore')
@observer
export class Works extends React.Component<any> {
    ...
    private renderToTaskLink = (): React.ReactNode => {
        if (this.props.user && this.work) {
            return (
                <Link to={`/user/${this.props.user.id}/courses/${this.course.id}/works/${this.work.id}/do`}>
                    Go to tast <DoubleRightOutlined />
                </Link>
            );
        } else return null;
    };
